...
import numpy as np
import os
df1 = pd.read_excel("F:\\AutoTest\\Python\\HLHD.xlsx") # 把excel内容读取为DataFrame对象
#print (df1)
#print "执行的excel中总共有%d列数据" %len(df1.columns)
#print "执行的excel中总共有%d行数据" %len(df1)
df2 = pd.read_excel("F:\\AutoTest\\Python\\lambor_Inputbox_HLHD.xlsx")
#print (df2)
...
time.sleep(3)
driver.get("http://backoffice.qc.lambor.ptg/Main/System/SystemBonusActivityAdd")
time.sleep(8)
...
df1.to_excel("F:\\AutoTest\\Python\\OUTHLHD.xlsx", index=False)  # 把df1另存为excel
