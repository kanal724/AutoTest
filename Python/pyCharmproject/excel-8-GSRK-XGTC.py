...
import numpy as np
import os
df1 = pd.read_excel("F:\\AutoTest\\Python\\GSRK-XGTC.xlsx") # 把excel内容读取为DataFrame对象
#print (df1)
#print "执行的excel中总共有%d列数据" %len(df1.columns)
#print "执行的excel中总共有%d行数据" %len(df1)
df2 = pd.read_excel("F:\\AutoTest\\Python\\lambor_Inputbox_GSRK-XGTC.xlsx")
#print (df2)
...
time.sleep(3)
driver.find_element_by_xpath("/html/body/div[1]/div[1]/form/fieldset/div[9]/button").click()  #点击登录按钮
time.sleep(3)
#driver.find_element_by_xpath("//*[@id=\"MasterLoginMessageModal\"]/div[1]").click()     #点击页面空白处取消掉提示弹窗
#time.sleep(5)
#driver.find_element_by_xpath("/html/body/div[1]/div[1]/div[1]/div/div/ul/li[4]/a").click()
#time.sleep(2)
driver.get("http://backoffice.qc.lambor.ptg/Main/System/SystemCompanyMoney")        #进入公司入款列表页面
time.sleep(2)
#driver.get("http://bkweb.qc.lambor.ptg/Main/System/SystemCompanyMoneyEdit/nxkHtsnVkBajDzxBCRgiLg==")
driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div/div/div[2]/table/tbody/tr[1]/td[2]/a").click()  #选择一个入款账户
time.sleep(3)
driver.find_element_by_xpath("/html/body/div[1]/div[2]/form/div/div/div/div[3]/a[3]").click()   #点击修改按钮
time.sleep(3)
#driver.find_element_by_xpath("//*[@id=\"myModal_edit_companybank\"]/div[2]/div[1]/div").click()
#time.sleep(3)
...
df1.to_excel("F:\\AutoTest\\Python\\OUTGSRK-XGTC.xlsx", index=False)  # 把df1另存为excel
