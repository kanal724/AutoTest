...
import numpy as np
import os
df1 = pd.read_excel("F:\\AutoTest\\Python\\HLHD-XGTC.xlsx") # 把excel内容读取为DataFrame对象
#print (df1)
#print "执行的excel中总共有%d列数据" %len(df1.columns)
#print "执行的excel中总共有%d行数据" %len(df1)
df2 = pd.read_excel("F:\\AutoTest\\Python\\lambor_Inputbox_HLHD-XGTC.xlsx")
#print (df2)
...
time.sleep(3)
driver.get("http://backoffice.qc.lambor.ptg/Main/System/SystemBonusActivity")
time.sleep(5)
driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div/div/div[2]/table/tbody/tr/td[1]/a").click()
time.sleep(20)
driver.find_element_by_xpath("/html/body/div[1]/div[2]/div[1]/div/div/div[3]/a[3]").click()
time.sleep(3)
...
for a in range(0,len(df2)):
    ...
    for index, word in enumerate(list(df1[inputbox_ID])): # 遍历df1中inputbox_ID下的所有值
        ...
        if word is np.nan:
            ...
            driver.find_element_by_xpath("//*[@id=\"MainModifyModal\"]/div[2]/div/div[1]/div").click()
            time.sleep(1)  # 等待
...
df1.to_excel("F:\\AutoTest\\Python\\OUTHLHD-XGTC.xlsx", index=False)  # 把df1另存为excel
