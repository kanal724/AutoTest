...
import numpy as np
import os
df1 = pd.read_excel("F:\\AutoTest\\Python\\HYCX-JJSX.xlsx") # 把excel内容读取为DataFrame对象
#print (df1)
#print "执行的excel中总共有%d列数据" %len(df1.columns)
#print "执行的excel中总共有%d行数据" %len(df1)
df2 = pd.read_excel("F:\\AutoTest\\Python\\lambor_Inputbox_HYCX-JJSX.xlsx")
#print (df2)
...
time.sleep(5)
driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div/div/div/div[2]/form/div[8]/div/button").click()
time.sleep(5)
...
for a in range(0,len(df2)):
    ...
    for index, word in enumerate(list(df1[inputbox_ID])): # 遍历df1中inputbox_ID下的所有值
        ...
        if word is np.nan:
            ...
            driver.find_element_by_xpath("//*[@id=\"myModal2\"]/form/div/div[1]").click()
            time.sleep(1)  # 等待
            dd = driver.find_element_by_xpath(tips_ID).get_attribute("class")
            ee = driver.find_element_by_xpath('//*[@id=\"myModal2\"]/form/div/div[2]/div[4]/div[1]/a[2]').get_attribute("class")
            tips_ID_ee = '//*[@id=\"myModal2\"]/form/div/div[2]/div[4]/div[1]/a[2]'
            c = '/span'
            m = tips_ID+c
            try:
            ...
            if a == False:
                result = "输入符合规范"
                print result
            elif a == True and tips_ID == '//*[@id=\"myModal2\"]/form/div/div[2]/div[4]/div[1]/a[1]' and ee == 'errorinfo':
                result = driver.find_element_by_xpath(tips_ID_ee).get_attribute("data-original-title")
...
df1.to_excel("F:\\AutoTest\\Python\\OUTHYCX-JJSX.xlsx", index=False)  # 把df1另存为excel
