...
import numpy as np
import os
df1 = pd.read_excel("F:\\AutoTest\\Python\\XZHY_XGTC_SJ.xlsx")   # 把excel内容读取为DataFrame对象

df2 = pd.read_excel("F:\\AutoTest\\Python\\lambor_Inputbox_XZHY_XGTC_SJ.xlsx")
...
time.sleep(2)
driver.find_element_by_xpath("/html/body/div[1]/div[3]/div/div/div/div/div[2]/div/a[4]").click()
time.sleep(5)
'''
...
for a in range(0, len(df2)):
    ...
    for index, word in enumerate(list(df1[inputbox_ID])):   # 遍历df1中inputbox_ID下的所有值
        ...
        if word is np.nan:
            ...
            if dd == cls:
                result = driver.find_element_by_xpath(tips_ID).get_attribute("data-original-title")
                print result
            elif dd == clsn:
                result = ""
                print "(无提示)"
...
df1.to_excel("F:\\AutoTest\\Python\\OUTXZHY_XGTC_SJ.xlsx", index=False)  # 把df1另存为excel
time.sleep(3)
driver.quit()
