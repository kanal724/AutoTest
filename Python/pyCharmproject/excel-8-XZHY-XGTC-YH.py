...
import numpy as np
import os
df1 = pd.read_excel("F:\\AutoTest\\Python\\XZHY-XGTC-YH.xlsx") # 把excel内容读取为DataFrame对象
#print (df1)
#print "执行的excel中总共有%d列数据" %len(df1.columns)
#print "执行的excel中总共有%d行数据" %len(df1)
df2 = pd.read_excel("F:\\AutoTest\\Python\\lambor_Inputbox_XZHY-XGTC-YH.xlsx")
#print (df2)
...
time.sleep(20)
driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div/div/form/div/ul/li[6]/a[1]").click()
time.sleep(3)
...
for a in range(0,len(df2)):
    ...
    for index, word in enumerate(list(df1[inputbox_ID])): # 遍历df1中inputbox_ID下的所有值
        ...
        if word is np.nan:
            ...
            driver.find_element_by_xpath("//*[@id=\"myModalModBank\"]/form/div/div/div[1]/div").click()
            time.sleep(1)  # 等待
...
df1.to_excel("F:\\AutoTest\\Python\\OUTXZHY-XGTC-YH.xlsx", index=False)  # 把df1另存为excel
