...
import numpy as np
import os
df1 = pd.read_excel("F:\\AutoTest\\Python\\XZDL_XGTC_SJ.xlsx")   # 把excel内容读取为DataFrame对象

df2 = pd.read_excel("F:\\AutoTest\\Python\\lambor_Inputbox_XZDL_XGTC_SJ.xlsx")
...
time.sleep(2)
driver.find_element_by_xpath("/html/body/div[1]/div[3]/div/div/div/div/div[1]/a[3]").click()
time.sleep(5)
'''
...
for a in range(0, len(df2)):
    ...
    for index, word in enumerate(list(df1[inputbox_ID])):   # 遍历df1中inputbox_ID下的所有值
        ...
        if word is np.nan:
            ...
            time.sleep(1)  # 等待
            driver.find_element_by_xpath("//*[@id=\"alertMofifyIB\"]/div[2]/div/div[1]").click()
            time.sleep(1)  # 等待
            '''if tips_ID == lw1:
                ddZd = driver.find_element_by_xpath(tips_ID + Zd).get_attribute("class")
                ddZ1 = driver.find_element_by_xpath(tips_ID + Z1).get_attribute("class")
...
df1.to_excel("F:\\AutoTest\\Python\\OUTXZDL_XGTC_SJ.xlsx", index=False)  # 把df1另存为excel
time.sleep(3)
driver.quit()
